...
const ProjectForm = ({ type, session, pr, id }) => {
  ...
  useEffect(() => {
    ...
    isValid2 && isValid1 && (currentimage.length > 0) ? setvalid(isValid2 && isValid1) : "";
  }, [form.liveSiteUrl, form.githubUrl,currentimage]);

  return (
    <form
      onSubmit={() => {
        setsub(true);
        toast.loading("uploading your project", { duration: 1000 });
      }}
      action={async (e) => {
        e.append("category", form.category);
        e.set("image",currentimage);
    ...
    >
      {/* <div className=" items-center  gap-4 max-md:flex-col relative flex max-w-7xl p-3 self-center">
        ...
        <label
          htmlFor="poster"
          className=" flexCenter max-sm:w-80 h-40 w-96 hover:border-white hover:text-primary-purple border-8 border-dashed text-2xl hover:scale-95 active:scale-90 transition-all   cursor-pointer border-violet-700 rounded-md"
        >
      ...
      {currentimage ? (
        <div className="relative h-48 w-full  mx-auto">
          ...
          <Image
            ...
            onClick={() => setImage("")}
            className=" p-1 
           absolute top-1 cursor-pointer  right-1 rounded-full rotate-45"
            src={"/plus-round.svg"}
          />
      ...
      <button
        ...
        disabled={sub || !valid}
        className={`${
          sub || !valid ? "!bg-gray border-0 !text-opacity-300 " : ""
        }  transition-all self-center  z-50 text-2xl dark:bg-white
            dark:text-primary-purple  flexCenter gap-2
            border-2 p-2 rounded-xl border-primary-purple`}
      >
        {sub && (
          <span className="h-10 rounded-full w-10 animate-spin border-dotted border-4 border-l-transparent rounde-full ">
            {" "}
          </span>
        )}{" "}
