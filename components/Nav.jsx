...
const   Nav = () => {
  ...
  <nav className='fixed group/main max-md:pt-7 border-b items-center  dark:border-none border-white max-md:text-3xl   bg-opacity-20 backdrop-blur-md 
   w-full max-md:px-8  px-16 top-0 left-0 z-20 flex justify-between pt-5  py-4'>
    
        <div className="absolute h-[2px] bg-gradient-to-r  transition-all bg-right
        group-hover/main:bg-left  bg-[length:300%]
        from-violet-900 to-white  w-full  bottom-0  left-0" />
        <Link href='/' className=" dark:hidden min-w-[100px] !text-gray-700">
            <Image height={30} width={90} src={flexible} alt='logo'  />
        </Link>
        <Link href='/' className=" min-w-[100px] hidden dark:flex !text-gray-700">
        ...
        <ul className="hidden items-end xl:flex  gap-4 my-2">
          {NavLinks.map(e=>(
            ...
            <li key={e.key} > 
            <Link  className='flex items-center text-lg overflow-hidden  whitespace-nowrap group h-10 hover:text-violet-500 transition  relative' href={e.href}>  {e.text}
            <div className="absolute h-1 -bottom-0 w-full transition-all   -left-full group-hover:left-0 bg-violet-600 "> </div> </Link> </li>
          ))}
